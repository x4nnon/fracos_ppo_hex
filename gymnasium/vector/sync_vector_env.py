...
class SyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self._total_steps_taken = np.zeros((self.num_envs,), dtype=np.float64)
    ...
    def fracos_step_async(self, action, next_obs, agent, total_rewards=0):
        ...
        for i, (env, action, obs) in enumerate(zip(self.envs, self._actions, next_obs)):            
            ...
            ) = env.fracos_step(action, obs, agent)
    ...
    # def fracos_step_async(self, action, next_obs, next_done, agent, 
    #                          obs, actions, 
    #                          logprobs, rewards, dones, values, total_rewards=0):
    #     """This needs to manage recursively taking actions"""
    #     # !!! Need to add in tracking of the steps -- unless this is done in the infos.
    #     self.step_async(action)
    #     observations, infos = [], {}
